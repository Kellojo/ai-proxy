...
export const GET: RequestHandler = async ({ request }) => {
  ...
  if (!bearer) {
    logRequest({
      model: "anthropic:models:list:unauthenticated",
      statusCode: 401,
      durationMs: Date.now() - startedAt,
    });
...
export const DELETE: RequestHandler = async ({ request }) => {
  ...
  if (!bearer) {
    logRequest({
      model: "anthropic:models:cache:clear:unauthenticated",
      statusCode: 401,
      durationMs: Date.now() - startedAt,
    });
  ...
  logRequest({
    providerId,
    model: providerId ? "anthropic:models:cache:clear:provider" : "anthropic:models:cache:clear",
    statusCode: 200,
    durationMs: Date.now() - startedAt,
