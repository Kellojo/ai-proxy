...
export const POST: RequestHandler = async ({ request }) => {
  ...
  const auth = requireVirtualKey(request, "responses:unauthenticated", startedAt);
...
async function handleAuthenticatedRequest(params: {
  ...
  const result = await attemptProviders(providers, params.body, forwardResponses, extractResponsesUsageMetrics, ctx);
